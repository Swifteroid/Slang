...
internal func shell(line command: String) throws -> String {
    try shell(command).trimmingTrailingCharacters(in: .newlines)
}
/// Runs the shell command and returns the entire stdout + stderr output truncated on the end and split into lines.
internal func shell(lines command: String) throws -> [String] {
    try shell(command).trimmingTrailingCharacters(in: .newlines).components(separatedBy: .newlines)
}
