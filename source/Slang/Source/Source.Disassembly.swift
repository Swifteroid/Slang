...
public class Disassembly {
    ...
    public func lines(in range: Range<Int>) -> [Line] {
        return lines.filter { range.overlaps($0.range) }
    }

    public func syntax(in range: Range<Int>) -> [Syntax] {
        return syntax.filter { range.overlaps($0.range) }
    }
}
...
extension Disassembly {
    ...
    public struct Query {
        ...
        public var fragment: FragmentQuery { return FragmentQuery(disassembly, [Fragment(disassembly.file)]) }
        public var syntax: SyntaxQuery { return SyntaxQuery(disassembly, disassembly.syntax) }
        public var structure: StructureQuery { return StructureQuery(disassembly, disassembly.structure) }
    }
}
...
extension FileSlice {
    ...
    public func lines(in disassembly: Disassembly) -> [Line] { return disassembly.lines(in: range) }

    /// Returns syntax tokens the receiver spans across in the disassembly.
    public func syntax(in disassembly: Disassembly) -> [Syntax] { return disassembly.syntax(in: range) }
}
